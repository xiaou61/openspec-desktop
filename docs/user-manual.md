...
| Node.js      | 仅从源码运行或开发时需要 Node.js 24；安装版和便携版不需要。                                                 |
| pnpm         | 仅从源码运行或开发时需要 pnpm 11，仓库锁定版本为 `11.16.0`。                                                |
| Git          | 获取源码时需要；应用也会只读使用项目的精确 Git 标签判断版本。项目没有标签或 Git 不可用时仍可浏览 OpenSpec。 |
| 网络         | 下载 Release、依赖或源码时需要。应用运行期间不上传项目路径、文档或历史。                                    |

从源码运行或需要严格验证时，在 PowerShell 中确认相应工具可用：
...
- SmartScreen 显示“未知发布者”：当前安装包没有代码签名；先确认下载地址和 SHA-256，无法确认来源时不要运行。
- 安装版无法启动：确认系统为 Windows 10/11 x64，并检查是否已有同一应用的窗口在后台运行；需要免安装排查时可改用同版本便携版。
...
1. 在目标环境安装或准备同版本应用，但先不要接入项目；可使用 Release 安装版、便携版或源码构建。
2. 关闭应用，把备份恢复到目标 user-data 目录；也可在启动当前 PowerShell 会话前指定一个恢复目录：
   ...
   上述命令适用于源码启动；安装版或便携版应先恢复到该版本应用已经显示过的 user-data 位置，再启动应用。
