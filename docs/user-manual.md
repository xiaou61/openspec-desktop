...
> 适用版本：OpenSpec Desktop `0.2.0` 的 `main` 源码更新（2026-08-14），不对应仓库既有的 `v0.2.0` Release 安装包。
...
本次更新通过公开仓库的 `main` 分支提供源码，不新建 GitHub Release、版本标签、安装程序、Portable 包或应用内帮助页。仓库既有的 `v0.2.0` Release、标签和安装包指向旧提交 `5f8d07b`，不包含本次 `main` 更新；要使用本手册描述的当前功能，请按本章从源码安装依赖并启动。
...
本次 `main` 源码更新没有配套的新安装包或自动卸载程序。退出应用后，可以删除本地源码检出及其 `node_modules/`、`out/` 等可再生内容。仓库历史 Release 安装包不包含本次更新，其卸载流程不在本手册范围内；项目根不在应用目录内，不会随应用卸载。
